...
<style>
    ...
    .stTabs [data-baseweb="tab-list"] { background-color: #1a1a2e; border-radius: 10px; padding: 4px; gap: 4px; }
    .stTabs [data-baseweb="tab"] { background-color: transparent; color: #a0a0c0; border-radius: 8px; padding: 8px 20px; font-weight: 500; }
...
with st.sidebar:
    ...
    selected_div = st.multiselect("Division", options=divisions[1:], default=divisions[1:])
...
COLORS  = ['#6c63ff','#ff6584','#43e97b','#f7971e','#00d2ff','#a18cd1','#fda085','#f6d365']
BGCOLOR  = '#0f0f1a'
PAPERBG  = '#1e1e3a'
FONT_CLR = '#c0c0e0'
GRID_CLR = '#2e2e4e'
...
with tab1:
    ...
    fig_lb = go.Figure()
    bar_colors = ['#6c63ff' if m >= overall_margin else '#ff6584' for m in sorted_prod['Gross Margin %']]
    fig_lb.add_trace(go.Bar(
        x=sorted_prod['Gross Margin %'], y=sorted_prod['Product Name'],
        orientation='h', marker_color=bar_colors,
        text=[f"{m:.1f}%" for m in sorted_prod['Gross Margin %']],
        textposition='outside'
    ))
    fig_lb.add_vline(x=overall_margin, line_dash="dash", line_color="#f7971e",
                     annotation_text=f"Avg {overall_margin:.1f}%", annotation_font_color="#f7971e")
    ...
    with col1:
        ...
        fig_pc = px.pie(product_df, values='Profit Contribution %', names='Product Name',
                        color_discrete_sequence=COLORS, hole=0.45)
        fig_pc.update_layout(paper_bgcolor=PAPERBG, font=dict(color=FONT_CLR), height=380,
                              margin=dict(l=20,r=20,t=30,b=20))
        fig_pc.update_traces(textposition='inside', textinfo='percent')
        st.plotly_chart(fig_pc, use_container_width=True)
    with col2:
        st.markdown("### 📦 Revenue Contribution %")
        fig_rc = px.pie(product_df, values='Revenue Contribution %', names='Product Name',
                        color_discrete_sequence=COLORS[::-1], hole=0.45)
        fig_rc.update_layout(paper_bgcolor=PAPERBG, font=dict(color=FONT_CLR), height=380,
                              margin=dict(l=20,r=20,t=30,b=20))
    ...
    fig_ppu = px.bar(ppu, x='Product Name', y='Profit per Unit', color='Division',
                     color_discrete_sequence=COLORS, text='Profit per Unit')
    fig_ppu.update_traces(texttemplate='$%{text:.2f}', textposition='outside')
    fig_ppu.update_layout(**base_layout("Profit per Unit"), height=380, xaxis_tickangle=-35)
    st.plotly_chart(fig_ppu, use_container_width=True)

    st.markdown("### 📋 Full Product Summary Table")
    display = product_df[['Product Name','Division','Total_Sales','Total_Profit',
    ...
    display.columns = ['Product','Division','Revenue ($)','Profit ($)','Units',
                       'Margin %','Profit/Unit ($)','Rev Contrib %','Profit Contrib %']
    st.dataframe(
        display.style
            .format({'Revenue ($)':'${:,.2f}','Profit ($)':'${:,.2f}',
                     'Profit/Unit ($)':'${:.2f}','Margin %':'{:.1f}%',
                     'Rev Contrib %':'{:.2f}%','Profit Contrib %':'{:.2f}%'})
...
with tab2:
    ...
    fig_rvp = go.Figure()
    fig_rvp.add_trace(go.Bar(name='Revenue', x=division_df['Division'], y=division_df['Total_Sales'],
                             marker_color='#6c63ff', text=[f"${v:,.0f}" for v in division_df['Total_Sales']],
                             textposition='outside'))
    fig_rvp.add_trace(go.Bar(name='Gross Profit', x=division_df['Division'], y=division_df['Total_Profit'],
                             marker_color='#43e97b', text=[f"${v:,.0f}" for v in division_df['Total_Profit']],
                             textposition='outside'))
    fig_rvp.add_trace(go.Bar(name='Cost', x=division_df['Division'], y=division_df['Total_Cost'],
                             marker_color='#ff6584', text=[f"${v:,.0f}" for v in division_df['Total_Cost']],
                             textposition='outside'))
    fig_rvp.update_layout(**base_layout("Revenue, Profit & Cost by Division"), barmode='group', height=400)
    st.plotly_chart(fig_rvp, use_container_width=True)

    col1, col2 = st.columns(2)
    with col1:
        st.markdown("### 📊 Gross Margin % by Division")
        fig_mg = px.bar(division_df.sort_values('Margin %', ascending=False),
                        x='Division', y='Margin %', color='Margin %',
                        color_continuous_scale='RdYlGn', text='Margin %')
        fig_mg.update_traces(texttemplate='%{text:.1f}%', textposition='outside')
        fig_mg.add_hline(y=overall_margin, line_dash='dash', line_color='#f7971e',
                         annotation_text=f"Overall {overall_margin:.1f}%", annotation_font_color='#f7971e')
        fig_mg.update_layout(**base_layout(), height=360, coloraxis_showscale=False)
        st.plotly_chart(fig_mg, use_container_width=True)
    with col2:
        st.markdown("### 🍩 Revenue Split by Division")
        fig_rsplit = px.pie(division_df, values='Total_Sales', names='Division',
                            color_discrete_sequence=COLORS, hole=0.5)
        fig_rsplit.update_layout(paper_bgcolor=PAPERBG, font=dict(color=FONT_CLR), height=360,
                                  margin=dict(l=20,r=20,t=30,b=20))
    ...
    fig_trend = px.line(monthly_df, x='Month', y='Margin %', color='Division',
                        color_discrete_sequence=COLORS, markers=True, line_shape='spline')
    fig_trend.add_hline(y=overall_margin, line_dash='dot', line_color='#f7971e',
                        annotation_text=f"Avg {overall_margin:.1f}%")
    fig_trend.update_layout(**base_layout("Gross Margin % Over Time"), height=380, xaxis_tickangle=-45)
    st.plotly_chart(fig_trend, use_container_width=True)

    st.markdown("### 🔍 Cost vs Sales Diagnostic")
    fig_scatter = px.scatter(product_df, x='Total_Sales', y='Total_Cost',
                             size='Total_Units', color='Division',
                             hover_name='Product Name', color_discrete_sequence=COLORS,
                             text='Product Name', size_max=60)
    ...
    fig_scatter.update_layout(**base_layout("Cost vs Sales — bubble size = units sold"), height=450)
    ...
    if not low_margin.empty:
        ...
        st.success("✅ All products exceed the margin threshold.")
...
with tab3:
    ...
    fig_par_rev.add_trace(go.Bar(x=pareto_rev['Product Name'], y=pareto_rev['Total_Sales'],
                                  name='Revenue', marker_color='#6c63ff',
                                  text=[f"${v:,.0f}" for v in pareto_rev['Total_Sales']],
                                  textposition='outside'), secondary_y=False)
    fig_par_rev.add_trace(go.Scatter(x=pareto_rev['Product Name'], y=pareto_rev['Cumulative Revenue %'],
                                      name='Cumulative %', mode='lines+markers',
                                      line=dict(color='#f7971e', width=2)), secondary_y=True)
    fig_par_rev.add_hline(y=80, line_dash='dash', line_color='#ff6584',
                           annotation_text='80% Line', secondary_y=True)
    fig_par_rev.update_layout(paper_bgcolor=PAPERBG, plot_bgcolor=BGCOLOR,
    ...
    fig_par_rev.update_yaxes(title_text="Cumulative %", secondary_y=True, range=[0,105])
    ...
    pareto_pft['Cumulative Profit %'] = (pareto_pft['Total_Profit'].cumsum() / pareto_pft['Total_Profit'].sum() * 100)
    fig_par_pft = make_subplots(specs=[[{"secondary_y": True}]])
    fig_par_pft.add_trace(go.Bar(x=pareto_pft['Product Name'], y=pareto_pft['Total_Profit'],
                                  name='Profit', marker_color='#43e97b',
                                  text=[f"${v:,.0f}" for v in pareto_pft['Total_Profit']],
                                  textposition='outside'), secondary_y=False)
    fig_par_pft.add_trace(go.Scatter(x=pareto_pft['Product Name'], y=pareto_pft['Cumulative Profit %'],
                                      name='Cumulative %', mode='lines+markers',
                                      line=dict(color='#f7971e', width=2)), secondary_y=True)
    fig_par_pft.add_hline(y=80, line_dash='dash', line_color='#ff6584',
                           annotation_text='80% Line', secondary_y=True)
    fig_par_pft.update_layout(paper_bgcolor=PAPERBG, plot_bgcolor=BGCOLOR,
                               font=dict(color=FONT_CLR), height=430,
                               margin=dict(l=40,r=40,t=50,b=80),
                               xaxis=dict(tickangle=-35, gridcolor=GRID_CLR),
                               legend=dict(bgcolor='rgba(0,0,0,0)'),
    ...
    fig_par_pft.update_yaxes(title_text="Cumulative %", secondary_y=True, range=[0,105])
    ...
    d3.metric("#1 Profit Driver", top1_product[:20], delta=f"{top1_pft_share:.1f}% of all profit")

    st.markdown("### 📉 Margin Volatility by Product")
    monthly_prod = df.groupby(['Month','Product Name']).agg(
        Sales=('Sales','sum'), Profit=('Gross Profit','sum')).reset_index()
    ...
    fig_vol = px.bar(volatility, x='Product Name', y='Margin Volatility (Std Dev)',
                     color='Margin Volatility (Std Dev)', color_continuous_scale='RdYlGn_r',
    ...
    fig_vol.update_layout(**base_layout("Margin Volatility — Higher = Less Predictable"),
                           height=370, xaxis_tickangle=-35, coloraxis_showscale=False)
    st.plotly_chart(fig_vol, use_container_width=True)

    st.markdown("### 🎯 Product Quadrant Matrix")
    fig_matrix = px.scatter(product_df, x='Revenue Contribution %', y='Gross Margin %',
                             size='Total_Profit', color='Division', hover_name='Product Name',
                             color_discrete_sequence=COLORS, text='Product Name', size_max=60)
    ...
    fig_matrix.update_layout(**base_layout("Product Quadrant: Revenue Contribution vs Gross Margin %"), height=480)
